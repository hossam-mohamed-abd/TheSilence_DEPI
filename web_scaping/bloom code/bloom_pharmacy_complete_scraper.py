#!/usr/bin/env python3
"""
Bloom Pharmacy Complete Web Scraper
Scrapes ALL categories and ALL products from the website
"""
...
class BloomPharmacyCompleteScraper:
    def __init__(self):
    ...
    def discover_all_categories(self) -> Dict[str, str]:
        ...
        try:
            # Try to get categories from collections page
    ...
    def extract_products(self, soup: BeautifulSoup, category: str) -> List[Dict]:
        ...
        for item in items[:100]:  # Safety limit per page
            try:
    ...
    def scrape_all(self):
        """Scrape ALL categories with ALL pages"""
        logger.info("\n" + "="*70)
        logger.info("BLOOM PHARMACY - COMPLETE WEBSITE SCRAPER")
        logger.info("="*70)
        ...
        for i, (name, slug) in enumerate(self.categories.items(), 1):
            logger.info(f"  {i}. {name} ({slug})")
...
def main():
    """Main execution"""
    scraper = BloomPharmacyCompleteScraper()

    try:
        # Scrape ALL categories and ALL pages
        scraper.scrape_all()
